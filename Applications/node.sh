...
# Get Starter File
wget -qO /tmp/scripts/starter https://raw.githubusercontent.com/FlarexCloud/FlarexCloud-Starter/main/starter.sh &> /dev/null
chmod +x /tmp/scripts/starter &> /dev/null # Giving Permissions
...
bash /tmp/scripts/starter "${PV_GIT_REPOSITORY}" "${PV_GIT_BRANCH}" "${PV_GIT_TOKEN}" ${PV_TERMINAL_MODE} ${PV_AUTO_PULL}
